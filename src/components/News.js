    ...
    return (
        <div>
            ...
            <div className="container my-3">
                <h2 style={{ marginTop: "4.5rem" }}>DailyBuzze-Top Headlines on : {props.category==="null"?"Today": capitalized(props.category)}</h2>
                {loading && <Spinner />}
                <div className="row">
                ...
                <div className="container" style={{ display: "flex", justifyContent: "space-between" }}>
                    ...
                    <button disabled={page >= Math.ceil(totalResult / props.pageSize)} type="button" className="btn btn-primary" onClick={handleNextpage}>Next &rarr;</button> {/* if we dont write this by default it assume as a props */}
                </div>
            </div>
        </div>
    )
